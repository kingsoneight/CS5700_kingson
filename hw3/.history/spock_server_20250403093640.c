...
/* Determine a single winning move among the array of moves, or -1 if tie.
 * This simplistic approach returns the index of a unique winning move if it
 * dominates all the others. If there's a tie or multiple winning moves, return -1.
 *
 * "Dominates" means it is not beaten by any other player's move.
 */
static int determine_multiplayer_winner(Move moves[], int numPlayers)
{
    /* For each player i, check if their move is "strictly dominant" = it beats
     * at least one move, and is not beaten by any move in that round.
     *
     * We'll use pairwise checks of the known RPSLS rules.  If exactly one
     * player's move passes that test, they're the winner. Otherwise, tie (-1).
     */

    /* A helper function to see if m1 beats m2 (like in two-player). */
    auto beats = [](Move m1, Move m2)
    {
    ...
    for (int i = 0; i < numPlayers; i++)
    {
        ...
        if (mi == MOVE_INVALID)
        {
            // A player didn't provide a valid move => skip
            continue;
        }

        // We'll check if "mi" is beaten by anyone else
        int is_beaten = 0;
        ...
        for (int j = 0; j < numPlayers; j++)
        {
            ...
            if (beats(mj, mi))
            {
                // someone else beats me => can't be winner
                is_beaten = 1;
                break;
            }
            if (beats(mi, mj))
        ...
        if (!is_beaten && at_least_one_beat)
        {
            ...
            {
                possible_winner_index = i; // first discovered possible winner
            }
            else
            {
                // we found more than one "dominant" move => tie
                return -1;
            }
        }
    }
